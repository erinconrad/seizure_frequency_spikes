...
if CFG.QUICK 
    fprintf(2, ['\n*** DRAFT RUN: nBoot=%d, model bootstrap=%d. ' ...
        'Numbers are NOT publishable. ***\n\n'], ...
        CFG.nBoot, CFG.nBootModel);
end
...
CFG.canonical3 = ["General","Temporal","Frontal"];  
...
assert_unique_keys(Spikes, "Patient",    "Session",        "Spikes"); % ensure no patient-session pair appears twice in spike table
assert_unique_keys(Report, "patient_id", "session_number", "Report"); % ensure no patient-session pair appears twice in clinical table

assert_same_keys_in_order(Spikes, "Patient",    "Session", ... % ensures that Spikes and Report contain the exact same patient-session pairs in exactly the same order
                          Report, "patient_id", "session_number", ...
                          "Spikes", "Report");

%% ===================== 3. BUILD COHORT =====================
Vuniq  = build_visit_level_table(Report);                          % 1 row / visit, has sz and sz freq per visit (27557 unique patientxvisit rows)
Typing = build_patient_typing_from_report(Report, CFG.canonical3); % 1 row / patient, epilepsy subtype
SzFreq = build_patient_seizure_metrics(Vuniq);                     % 1 row / patient, mean sz frequency across visits
...
%                 Fig 2  spike rate for diff patient types and EEGs with and without spikes
...
%                 Fig S4  generalized-epilepsy subtypes
...
[GenSub, FigGenSub] = make_fig_generalized_subtypes(Views, CFG, ...
    f('FigS4_generalized_subtypes.png'), genSubMin, false);

% ---- Fig 2: detector controls and spikes across epilepsy subtypes ----
...
% Fig S6: same figure on the cohort with the top spikers dropped. The cutoff is
% set on the all-epilepsy table and applied to both, so the two panels' cohorts
% stay consistent with each other.
trimFrac = 0.10;
spikeThr = prctile(Views.PatientSpikeSz_All.MeanSpikeRate_perHour, 100*(1-trimFrac));
SP.trim  = spearman_figure( ...
    Views.PatientSpikeSz_All(Views.PatientSpikeSz_All.MeanSpikeRate_perHour     <= spikeThr, :), ...
    Views.PatientSpikeSz_Typed(Views.PatientSpikeSz_Typed.MeanSpikeRate_perHour <= spikeThr, :), ...
    CFG, f('FigS6_spearman_trim10.png'), ...
    sprintf(' (top %.0f%% spikers removed)', 100*trimFrac), false);
SP.trim.trimInfo = struct('trimFrac', trimFrac, 'threshold', spikeThr, ...
    'nRemovedAll',   nnz(Views.PatientSpikeSz_All.MeanSpikeRate_perHour   > spikeThr), ...
    'nRemovedTyped', nnz(Views.PatientSpikeSz_Typed.MeanSpikeRate_perHour > spikeThr));

% ---- Fig S7: sz freq by reported spikes ----
...
Supp = struct('Duration',DurStats, 'Bias',BiasStats, 'GenSub',GenSub, ...
              'Trim',SP.trim, 'DurModel',DurCompare);
...
for i = 1:height(R)
    ...
    % If the visit type is missing, then set all info about it to be
    % missing
    ...
    keep    = ismember(string(vt), allowableVisits); % see if the visit type is an allowable (outpatient) visit

    % if no visit types are allowable, set all visit info to be empty
    if ~any(keep)
        [R.visit_type(i), R.visit_dates_deid(i), ...
         R.sz_freqs(i),   R.visit_hasSz(i)] = deal("[]");
        continue
    ...
    % only keep sz frequencies, etc. for allowable visit types
...
% outpatient if it's acquired on one of the outpatient machines, or if the
% report explicitly says outpatient
...
Keys = innerjoin(OutptKeys, RoutineKeys, 'Keys', {'Patient','Session'}); % combine outpatient and routine requirements
S = innerjoin(S, Keys, 'Keys', {'Patient','Session'}); % restrict spike info to be outpatient routine
R = innerjoin(R, Keys, 'LeftKeys', {'patient_id','session_number'}, ... % restrict clinic info to be outpatient and routine
...
% Rows is a cell array the same length as R, and each element of rows
% contains a table with as many rows as there are clinic visits for that
% patient, along with visitDate, freq and Has Sz

PV = vertcat(PV, rows{~cellfun(@isempty, rows)}); % PV is now a table with the above structure

% Collapse duplicated (patient, date) rows. Duplicates come from the same
% visit being listed on several EEG rows.
% This code takes the mean across identical patient-date instances (they
% should be the same)
...
% This double checks that they are indeed the same
nDisagree = nnz(splitapply(@(x) numel(unique(x(isfinite(x)))) > 1, PV.Freq, g));
assert(nDisagree == 0, ...
    '%d (patient, visit) groups have conflicting seizure frequencies across EEG rows.', ...
    nDisagree);

% This builds a table without duplicate Patient-visit date combos, so it
% really is 1 unique visit per row
Vuniq = table(pid, dt, Freq, HasSz, ...
    'VariableNames', {'Patient','VisitDate','Freq','HasSz'});

% applies rule 1 - if no listed freq but has sz == 0 -> freq = 0;
Vuniq.Freq_R1 = Vuniq.Freq;
r1 = ~isfinite(Vuniq.Freq_R1) & (Vuniq.HasSz == 0);
Vuniq.Freq_R1(r1) = 0;

% applies rule 2 - if no listed has sz but frew == 0 -> has sz = 0;
...
% Erin checked these rules and they look ok
...
[g, pids] = findgroups(Vuniq.Patient); % Group by patients
MeanSzFreq = splitapply(@(x) mean(x,'omitnan'), Vuniq.Freq_R1, g); % mean sz frequency across visits for that patient
...
E3((strlength(E3)==0) & strcmp(type,"general")) = "General"; % assign to general if not already assigned to temporal/frontal and if it's general
T.EpiType3 = categorical(E3, canonical3);

% Erin checked these. All "General" patients appear to have
% generalized-sounding EpilepsySpecific. I EXCLUDED combined generalized
% and focal by requiring the type to be exactly general.

% Frontal and Temporal look good too, noting that a few of these had
% "combined generalized and focal" designations, which I will allow.
end
...
assert(all(ismember(Typing.Patient, PatientsKept)), ...
    'Typing covers %d patients absent from the filtered spike table.', ...
    nnz(~ismember(Typing.Patient, PatientsKept)));
TypingKept = Typing;
...
nWithEpilepsy = numel(EpiPatients); % Erin checked this looks good. It includes "unclassified or unspecified" patients, which is fine.
...
Canonical3_SubsetTable.Properties.VariableNames{'EpiType3'} = 'EpiType4'; % epitype4 is an old naming convention
...
% Build per patient and concatenate once.
...
T = PairTable(keep, :); % now this removes the ones where HasSz == nan
...
% Restrict to patients with short gap and long gap visits
...
%
% The medians and their bootstrap intervals are returned as well as drawn, so
% the Fig. S3 legend quotes exactly the numbers that are plotted rather than
% recomputing them from a different bootstrap draw.
x1 = val(grp == grpNames(1)); x1 = x1(isfinite(x1));
x2 = val(grp == grpNames(2)); x2 = x2(isfinite(x2));
p  = NaN; d = NaN;
if ~isempty(x1) && ~isempty(x2)
    p = ranksum(x1, x2, 'method','approx');
    d = cliff_delta(x1, x2);          % > 0 means group 1 tends to exceed group 2
end
...
[med, lo, hi] = deal(nan(1,2));
for k = 1:2
    [med(k), lo(k), hi(k)] = bootstrap_median_ci(val(grp == grpNames(k)), ...
        CFG.nBoot, CFG.alpha);
    add_median_ci_overlay(ax, k, med(k), lo(k), hi(k), epsFloor);
end
...
stats = struct('p_ranksum',p, 'cliff_delta',d, 'n1',numel(x1), 'n2',numel(x2), ...
    'med1',med(1), 'lo1',lo(1), 'hi1',hi(1), ...
    'med2',med(2), 'lo2',lo(2), 'hi2',hi(2));
end
...
%
% Subtypes come from the free-text epilepsy_specific field. Only subtypes with
% STRICTLY MORE than minN patients are plotted; subtypes at or below that size
% are dropped outright (no pooling into an "Other generalized" bucket), and no
% pooled all-generalized panel is drawn.
%
% GenSub now accounts for EVERY generalized patient rather than only the
% plotted ones: one row per syndrome carrying N, whether it was plotted, and
% the panel letter it was drawn in, plus an "All generalized" summary row.
% The Fig. S4 legend in the results HTML is written from this table, so the
% legend cannot disagree with the figure. Correlations are left NaN for
% syndromes that were not plotted.
%
% Blank / missing free text is labelled "<blank>" instead of being silently
% dropped by findgroups. It is never plotted (which is also what happened
% before), but it now appears in the accounting.
...
D.Subtype = strtrim(string(D.EpilepsySpecific));
D.Subtype(ismissing(D.Subtype) | D.Subtype == "") = "<blank>";

%% --- Counts for every syndrome ---
[g1, lab1] = findgroups(D.Subtype);
n1 = splitapply(@numel, D.Subtype, g1);
assert(sum(n1) == height(D), ...
    'Syndrome counts (%d) do not account for every generalized patient (%d).', ...
    sum(n1), height(D));

% Inventory, so unmapped vocabulary is visible rather than silently dropped.
fprintf('\n[Generalized subtypes] epilepsy_specific inventory (N=%d patients)\n', height(D));
for i = 1:numel(lab1)
    fprintf('  %-45s N=%d\n', lab1(i), n1(i));
end

%% --- Keep only syndromes above the size threshold (no pooling) ---
keep = (n1 > minN) & (lab1 ~= "<blank>");
labs = lab1(keep);
nPer = n1(keep);
[nPer, ord] = sort(nPer, 'descend'); labs = labs(ord);

% Keep the catch-all bucket last regardless of size.
isLast = ismember(labs, "Unspecified generalized");
labs = [labs(~isLast); labs(isLast)];
nPer = [nPer(~isLast); nPer(isLast)];
nG   = numel(labs);
assert(nG >= 1, 'No generalized subtype has more than %d patients (largest N=%d).', ...
    minN, max(n1));

inPanel = ismember(D.Subtype, labs);
assert(sum(nPer) == sum(inPanel), 'Panel counts do not match the number of plotted patients.');

fprintf('[Generalized subtypes] %d panels with N>%d; %d/%d patients shown\n', ...
    nG, minN, sum(nPer), height(D));

%% --- Correlations (Bonferroni over the plotted syndromes) ---
...
%% --- One row per syndrome (plotted or not) + an all-generalized summary ---
[nAll, ordAll] = sort(n1, 'descend');
labAll = lab1(ordAll);

panel = strings(numel(labAll),1);
[rhoA, loA, hiA, pRawA, pBonfA] = deal(nan(numel(labAll),1));
idxPlotted = zeros(nG,1);
for k = 1:nG
    idxPlotted(k) = find(labAll == labs(k), 1);
    panel(idxPlotted(k))  = string(char('A'+k-1));
    rhoA(idxPlotted(k))   = rho(k);
    loA(idxPlotted(k))    = lo(k);
    hiA(idxPlotted(k))    = hi(k);
    pRawA(idxPlotted(k))  = p_raw(k);
    pBonfA(idxPlotted(k)) = p_bonf(k);
end

% Plotted syndromes first, in panel order; everything else by descending N.
idxRest  = setdiff((1:numel(labAll))', idxPlotted, 'stable');
ordFinal = [idxPlotted; idxRest];

GenSub = table( ...
    ["All generalized"; labAll(ordFinal)], ...
    [height(D);         nAll(ordFinal)], ...
    [false;             ismember(ordFinal, idxPlotted)], ...
    ["";                panel(ordFinal)], ...
    [rho_all;           rhoA(ordFinal)], ...
    [lo_all;            loA(ordFinal)], ...
    [hi_all;            hiA(ordFinal)], ...
    [p_all;             pRawA(ordFinal)], ...
    [NaN;               pBonfA(ordFinal)], ...
    'VariableNames',{'Group','N','Plotted','Panel','Spearman_rho', ...
                     'ci_lo','ci_hi','p_raw','p_bonf'});

GenSub.Properties.UserData = struct('minN', minN, 'nGeneralized', height(D), ...
    'nPlotted', sum(nPer), 'nSyndromes', numel(labAll), 'nPanels', nG, ...
    'nonZeroOnly', nonZeroOnly);

assert(sum(GenSub.N(2:end)) == height(D), ...
    'GenSub syndrome rows (%d patients) do not sum to the generalized total (%d).', ...
    sum(GenSub.N(2:end)), height(D));
assert(sum(GenSub.N(GenSub.Plotted)) == sum(inPanel), ...
    'GenSub plotted rows do not match the number of plotted patients.');

%% --- Draw ---
[eps_sz, eps_rate] = zero_floors(D.MeanSzFreq(inPanel), D.MeanSpikeRate_perHour(inPanel));
...
for k = 1:nG
    m = (D.Subtype == labs(k));
    draw_spearman_panel(nexttile(k), D.MeanSzFreq(m), D.MeanSpikeRate_perHour(m), ...
        COL(mod(k-1,size(COL,1))+1,:), ...
        sprintf('%s. %s (N=%d)', char('A'+k-1), labs(k), nPer(k)), ...
...
fprintf(fid, ['<p>Of %d patients with EEG data in the Penn Epilepsy Center database, ' ...
    ...
    'Most patients had only a single EEG throughout their follow up. ' ...
    'Median EEG duration was %.0f minutes (IQR %.0f&ndash;%.0f; Fig. S2). ' ...
    '%d patients (%.1f%%) had spikes reported on at least ' ...
    'one EEG. '...
    'Median [95%% CI] monthly seizure frequency was %.2f [%.2f-%.2f], and median ' ...
...
fprintf(fid, ['EEGs for which the reported presence or absence of spikes was ' ...
    ...
    'detected spike rate, seizure frequency, ' ...
    'and epilepsy subtype distribution ' ...
    '(Fig. S3). '], ...
...
fprintf(fid, 'Correlations were similar within individual generalized-epilepsy syndromes (Fig. S4). ');


temNZ = getG(SP.nz.Results, "Temporal");
frontNZ = getG(SP.nz.Results, "Frontal");
fprintf(fid, ['When restricting to patients with non-zero spike rates and seizure ' ...
    ...
    'magnitude (&rho;=%.2f [%.2f-%.2f]), and the correlation for '...
    'frontal lobe epilepsy grew, although remained non-significant (&rho;=%.2f [%.2f-%.2f], '...
    'Bonferroni-adjusted %s; Fig. S5). '], ...
    format_p_html(temNZ.p_bonf), temNZ.Spearman_r, temNZ.ci_lo, temNZ.ci_hi,...
    frontNZ.Spearman_r, frontNZ.ci_lo, frontNZ.ci_hi,format_p_html(frontNZ.p_bonf));

assert(isfield(Supp,'Trim') && isfield(Supp.Trim,'trimInfo'), ...
    'Supp.Trim.trimInfo is missing; set it on SP.trim after the Fig S6 call.');
TI = Supp.Trim.trimInfo;
fprintf(fid, ['After excluding the %.0f%% of patients with the highest spike rates ' ...
    '(above %.1f spikes/hour; N=%d patients excluded), the sign and significance of the ' ...
    'correlations were unchanged, although the correlations became smaller for all ' ...
    'epilepsy patients, frontal, and generalized patients ' ...
    '(Fig. S6). '], ...
    100*TI.trimFrac, TI.threshold, TI.nRemovedAll);
...
fprintf(fid, ['<p>Seizure frequency varies over time within individuals, and we ' ...
    ...
    'model fit over a model without them (%s).'], ...
    height(MMR.ModelTable), numel(unique(MMR.ModelTable.Patient)), ...
    format_lrt_html(MMR.LRT));

% EEG duration as a covariate, refit on identical rows so the LRT is valid.
assert(isfield(Supp,'DurModel'), ...
    'Supp.DurModel is missing; pass DurCompare into write_results_html.');
DM = Supp.DurModel;
assert(isscalar(DM.Duration_OR) && isfinite(DM.Duration_p), ...
    'EEG duration coefficient absent from the duration-model fixed effects.');

assert(DM.LRT_p >= CFG.alpha, ...
    'EEG duration now improves model fit (LRT p = %.4g); the hardcoded sentence is wrong.', ...
    DM.LRT_p);

fprintf(fid, [' A secondary model adding EEG duration as a covariate to this primary model ' ...
    'did not improve model fit (%s), suggesting that the spike-seizure association is not ' ...
    'explained by recording length.</p>\n'], ...
    format_lrt_html(DM.LRT));
...
%% ---------------- Fig S3 legend ----------------
function write_bias_legend(fid, BiasStats, CFG)
%WRITE_BIAS_LEGEND  Fig. S3 legend, with every number taken from BiasStats.
%
% Fig. S3 asks whether EEGs WITHOUT an interpretable spike report differ from
% those with one. The verb in the headline is chosen from the p-values rather
% than asserted, so a significant difference cannot be written up as similarity.
% Subtype labels come from CFG.canonical3, the same source the figure's column
% order comes from, so the two cannot drift apart.

assert(all(isfield(BiasStats, {'nWithReport','nNoReport','SpikeRate','SzFreq','EpiType'})), ...
    'BiasStats is missing fields required for the Fig. S3 legend.');
SR = BiasStats.SpikeRate;
SF = BiasStats.SzFreq;
E  = BiasStats.EpiType;
assert(all(isfield(SR, {'med1','lo1','hi1','med2','lo2','hi2'})) && ...
       all(isfield(SF, {'med1','lo1','hi1','med2','lo2','hi2'})), ...
    ['BiasStats carries no medians; regenerate it with the updated ' ...
     'two_group_panel.']);
assert(size(E.Counts,2) == numel(CFG.canonical3), ...
    'Fig. S3 contingency table has %d columns but %d canonical subtypes.', ...
    size(E.Counts,2), numel(CFG.canonical3));

nC   = sum(E.Counts(:));
assert(nC > 0, 'Fig. S3 contingency table is empty.');

verb = similar_or_differed([SR.p_ranksum, SF.p_ranksum, E.p], CFG.alpha);
subtypeStr = strjoin(lower(string(CFG.canonical3)), ", ");
warnstr = '';
if any(E.Expected(:) < 5), warnstr = ' (one or more expected counts below 5)'; end

fprintf(fid, '<h2>Figure S3 legend</h2>\n');
fprintf(fid, ['<p><strong>Fig. S3. EEGs without an interpretable report of spike ' ...
    'presence %s those with one.</strong> Each EEG in the study cohort was classified ' ...
    'by whether its clinical report contained an interpretable statement about the ' ...
    'presence or absence of interictal spikes (N = %d EEGs with a report, N = %d ' ...
    'without). <strong>A:</strong> automatically detected spike rate, on a logarithmic ' ...
    'axis (median %.2f [95%% CI %.2f&ndash;%.2f] spikes/hour with a report vs ' ...
    '%.2f [%.2f&ndash;%.2f] without; %s, Cliff''s &delta; = %.2f). ' ...
    '<strong>B:</strong> mean documented seizure frequency of the patient contributing ' ...
    'each EEG (median %.2f [%.2f&ndash;%.2f] vs %.2f [%.2f&ndash;%.2f] seizures/month; ' ...
    '%s, Cliff''s &delta; = %.2f). <strong>C:</strong> epilepsy subtype (%s) for the ' ...
    '%d EEGs from patients with one of these subtypes; each cell gives the number of ' ...
    'EEGs and the percentage of its row (&chi;&sup2;(%d) = %.1f, %s)%s. ' ...
    'In A and B, boxes show the median and interquartile range, points are individual ' ...
    'EEGs, and the black marker and vertical line give the median with its 95%% ' ...
    'bootstrap confidence interval; values of zero are drawn at a floor below the ' ...
    'smallest positive value on that axis (dotted line). Groups were compared with ' ...
    'Wilcoxon rank-sum tests in A and B, with Cliff''s &delta; positive when EEGs with ' ...
    'an available report take the higher values, and with a chi-square test of ' ...
    'independence in C. All three panels are at the EEG level, while the seizure ' ...
    'frequency in B and the subtype in C are patient-level values, so a patient ' ...
    'contributing several EEGs is counted several times; these comparisons are ' ...
    'descriptive and their p values do not account for within-patient clustering.</p>\n'], ...
    verb, BiasStats.nWithReport, BiasStats.nNoReport, ...
    SR.med1, SR.lo1, SR.hi1, SR.med2, SR.lo2, SR.hi2, ...
    format_p_html(SR.p_ranksum), SR.cliff_delta, ...
    SF.med1, SF.lo1, SF.hi1, SF.med2, SF.lo2, SF.hi2, ...
    format_p_html(SF.p_ranksum), SF.cliff_delta, ...
    subtypeStr, nC, E.df, E.chi2, format_p_html(E.p), warnstr);
end

%% ---------------- Fig S3 legend ----------------
write_bias_legend(fid, Supp.Bias, CFG);

%% ---------------- Fig S4 legend ----------------
write_genSub_legend(fid, Supp.GenSub);
...
function write_genSub_legend(fid, GenSub)
%WRITE_GENSUB_LEGEND  Fig. S4 legend, with the patient count for every
% generalized-epilepsy syndrome.
%
% Every number comes from GenSub, which carries one row per syndrome plus an
% "All generalized" summary row, so the legend cannot disagree with the
% figure. Syndromes below the size threshold were not plotted but ARE listed,
% so the counts account for every patient in the generalized group.

require_cols(GenSub, ["Group","N","Plotted","Panel","Spearman_rho", ...
                      "ci_lo","ci_hi","p_raw"], "GenSub");
UD = GenSub.Properties.UserData;
assert(isstruct(UD) && isfield(UD,'minN'), ...
    'GenSub has no UserData.minN; regenerate it with the updated make_fig_generalized_subtypes.');

isAll = (GenSub.Group == "All generalized");
assert(nnz(isAll) == 1, 'GenSub must contain exactly one "All generalized" summary row.');
Row = GenSub(isAll, :);
Sub = GenSub(~isAll, :);
assert(sum(Sub.N) == Row.N, ...
    'Per-syndrome counts (%d) do not sum to the generalized total (%d).', ...
    sum(Sub.N), Row.N);

P  = Sub(Sub.Plotted,  :);      % already in panel order
NP = Sub(~Sub.Plotted, :);      % descending N

plotStr = strjoin(arrayfun(@(k) sprintf('<strong>%s:</strong> %s (N = %d)', ...
    P.Panel(k), P.Group(k), P.N(k)), (1:height(P))', 'UniformOutput', false), '; ');

if isempty(NP)
    notStr = 'no syndrome fell below this threshold';
else
    notStr = strjoin(arrayfun(@(k) sprintf('%s (N = %d)', NP.Group(k), NP.N(k)), ...
        (1:height(NP))', 'UniformOutput', false), '; ');
end

fprintf(fid, '<h2>Figure S4 legend</h2>\n');
fprintf(fid, ['<p><strong>Fig. S4. The association between interictal spike rate and ' ...
    'seizure frequency within syndromes of generalized epilepsy.</strong> ' ...
    'Patients with generalized epilepsy contributing both a mean spike rate and a mean ' ...
    'documented seizure frequency (N = %d) were subclassified by the large language model, '...
    'yielding %d syndromes. Syndromes with more than %d patients ' ...
    'are shown, one per panel (%d panels, %d of %d patients): %s. Each point is one ' ...
    'patient, plotting that patient''s mean spike rate across EEGs against mean documented ' ...
    'seizure frequency across clinic visits, on logarithmic axes. Patients with a spike ' ...
    'rate or seizure frequency of zero are drawn as asterisks at a floor half a step below ' ...
    'the smallest positive value on that axis (dotted lines); fit lines use only patients ' ...
    'with non-zero values on both axes. Each panel reports the Spearman correlation with ' ...
    'its 95%% bootstrap confidence interval and a p value Bonferroni-adjusted across the ' ...
    '%d syndromes shown. Syndromes with %d or fewer patients were not plotted and are ' ...
    'listed here for completeness: %s. </p>\n'], ...
    Row.N, height(Sub), UD.minN, height(P), sum(P.N), Row.N, plotStr, ...
    height(P), UD.minN, notStr);
end
...
function s = format_lrt_html(lrt)
%FORMAT_LRT_HTML  "chi2(df) = stat, p = ..." from a compare() results table.
% df is read from the table rather than hardcoded at the call site, so a change
% to the model formulas cannot leave a stale df in the results text.
%
% compare() on a GLME returns a legacy dataset array, not a table, so convert
% first; dataset exposes VarNames where require_cols expects VariableNames.
if isa(lrt, 'dataset'), lrt = dataset2table(lrt); end
require_cols(lrt, ["LRStat","deltaDF","pValue"], "LRT table");
assert(height(lrt) == 2, 'Expected a two-row compare() result, got %d rows.', height(lrt));
s = sprintf('&chi;&sup2;(%d) = %.1f, %s', ...
    lrt.deltaDF(2), lrt.LRStat(2), format_p_html(lrt.pValue(2)));
end


function assert_same_keys_in_order(A, aPid, aSes, B, bPid, bSes, nameA, nameB)
%ASSERT_SAME_KEYS_IN_ORDER  A and B hold the same (patient, session) pairs, in
% the same row order. Assumes both have already passed assert_unique_keys:
% the membership check below dedupes, so a duplicated key would slip past it.
%
% The last two checks use if/error rather than assert because assert evaluates
% its message arguments unconditionally, and those messages index into results
% that are empty on the passing path.
keyA = string(A.(aPid)) + "|" + string(A.(aSes));
keyB = string(B.(bPid)) + "|" + string(B.(bSes));

assert(numel(keyA) == numel(keyB), ...
    '%s has %d rows but %s has %d.', nameA, numel(keyA), nameB, numel(keyB));

onlyA = setdiff(keyA, keyB);
onlyB = setdiff(keyB, keyA);
if ~isempty(onlyA) || ~isempty(onlyB)
    error(['%s and %s cover different (patient, session) pairs: ' ...
        '%d only in %s (%s), %d only in %s (%s).'], ...
        nameA, nameB, numel(onlyA), nameA, key_examples(onlyA), ...
        numel(onlyB), nameB, key_examples(onlyB));
end

bad = find(keyA ~= keyB);
if ~isempty(bad)
    error(['%s and %s hold the same %d pairs but in a different row order; ' ...
        'first difference at row %d (%s vs %s).'], ...
        nameA, nameB, numel(keyA), bad(1), keyA(bad(1)), keyB(bad(1)));
end
end


function s = key_examples(k)
